...
#if BASETEXTURE
				in Texture2D BaseTextureSampler, in SamplerState BaseTextureSampler_s,
			    in Texture2D LightmapSampler,    in SamplerState LightmapSampler_s,
#endif
				in Texture2D NormalSampler,   in SamplerState NormalSampler_s,
			    in Texture2D RefractSampler,  in SamplerState RefractSampler_s,
			    in Texture2D ReflectSampler,  in SamplerState ReflectSampler_s,
			    out float4 result, out float fogFactor )
{
...
#if MULTITEXTURE
	float4 vNormal  = NormalSampler.Sample( NormalSampler_s, i.vBumpTexCoord );
	float4 vNormal1 = NormalSampler.Sample( NormalSampler_s, i.vExtraBumpTexCoord.xy );
	float4 vNormal2 = NormalSampler.Sample( NormalSampler_s, i.vExtraBumpTexCoord.zw );
...
#else
	float4 vNormal = DecompressNormal( NormalSampler, NormalSampler_s, i.vBumpTexCoord, NORMAL_DECODE_MODE );
#endif
...
#if ABOVEWATER
	float waterFogDepthValue = RefractSampler.Sample( RefractSampler_s, unwarpedRefractTexCoord ).a;
#else
	...
	float4 vReflectColor = ReflectSampler.Sample( ReflectSampler_s, vReflectTexCoord );
#if BLURRY_REFRACT
	...
	for(int ix=-2;ix<=2;ix++)
	{
		for(int iy=-2;iy<=2;iy++)
		{
			...
			vRefractColor += weight*RefractSampler.Sample( RefractSampler_s, vRefractTexCoord+ix*ddx1+iy*ddy1);
			sumweights+=weight;
		}
	}
#else
	...
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -2 * ddx1 + -2 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -2 * ddx1 + -1 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -2 * ddx1 + 0 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -2 * ddx1 + 1 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -2 * ddx1 + 2 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -1 * ddx1 + -2 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -1 * ddx1 + -1 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -1 * ddx1 + 0 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -1 * ddx1 + 1 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + -1 * ddx1 + 2 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + 0 * ddx1 + -2 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + 0 * ddx1 + -1 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + 0 * ddx1 + 0 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + 0 * ddx1 + 1 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + 0 * ddx1 + 2 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + 1 * ddx1 + -2 * ddy1 );
	vRefractColor += RefractSampler.Sample( RefractSampler_s, vRefractTexCoord + 1 * ddx1 + -1 * ddy1 );
...
#else
	vReflectColor *= i.vReflectTint;
	float4 vRefractColor = RefractSampler.Sample( RefractSampler_s, vRefractTexCoord );
	// get the depth value from the refracted sample to be used for fog.
#	if ABOVEWATER
	// Don't mess with this in the underwater case since we don't really have
	// depth values there.
	waterFogDepthValue = RefractSampler.Sample( RefractSampler_s, vRefractTexCoord ).a;
#	endif
#endif
...
#if BASETEXTURE
	float4 baseSample = BaseTextureSampler.Sample( BaseTextureSampler_s, i.vBumpTexCoord.xy );
	float2 bumpCoord1;
	float2 bumpCoord2;
	float2 bumpCoord3;
	...
	float4 lightmapSample1 = LightmapSampler.Sample( LightmapSampler_s, bumpCoord1 );
	float3 lightmapColor1 = lightmapSample1.rgb;
	float3 lightmapColor2 = LightmapSampler.Sample( LightmapSampler_s, bumpCoord2 ).rgb;
	float3 lightmapColor3 = LightmapSampler.Sample( LightmapSampler_s, bumpCoord3 ).rgb;
